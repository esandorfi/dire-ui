...
export class CardTransformerService {
  ...
  private transformNodeToCards(node: Node, index: number): CardCollection {
    switch (node.type) {
      ...
      case 'philosophy_statement':
        return [this.transformPhilosophyStatement(node as PhilosophyStatement, index)];
      case 'main_offer':
      ...
      case 'modalites_formations':
        return [this.transformModalitesFormations(node as ModalitesFormations, index)];
      case 'footer_info':
   ...
   */
  private transformPhilosophyStatement(node: PhilosophyStatement, index: number): PhilosophyCard {
    return {
      id: `philosophy-${index}`,
      type: 'philosophy',
   ...
   */
  private transformFormationsList(node: FormationsList, index: number): FormationCard[] {
   ...
   */
  private transformFormation(formation: Formation, index: string): FormationCard {
    return {
      id: `formation-${index}`,
      type: 'formation',
   ...
   */
  private transformAuthorProfile(node: AuthorProfile, index: number): ProfileCard {
    return {
      id: `profile-${index}`,
      type: 'profile',
   ...
   */
  private transformModalitesFormations(node: ModalitesFormations, index: number): DetailsCard {
    const sections = [
      { title: node.details.public.title, content: node.details.public.content },
      { title: node.details.pedagogie.title, content: node.details.pedagogie.content },
      { title: node.details.groupe.title, content: node.details.groupe.content },
      { title: node.details.duree.title, content: node.details.duree.content },
      { title: node.details.session.title, content: node.details.session.content }
    ];
